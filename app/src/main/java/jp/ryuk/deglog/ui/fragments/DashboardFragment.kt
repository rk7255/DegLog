...
import jp.ryuk.deglog.ui.data.OnCallKey
...
class DashboardFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.type.observe(viewLifecycleOwner, Observer {
            binding.dbPersonalIcon.setImageResource(Utils.iconSelector(requireContext(), it))
        })
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.toolbar_add -> {
                this.findNavController().navigate(
                    DashboardFragmentDirections.actionDiaryFragmentToNewDiaryFragment(
                        "new",
                        -1,
                        viewModel.selected.value ?: ""
                    )
